 ...
 */
#define IADC_INPUT_0_PORT_PIN     iadcPosInputPortAPin5; // working pin iadcPosInputPortAPin7 use PA8 (H3) for PWM out
...
volatile uint8_t _sample_flag = 0;
const float MAX_RANGE = 1.5;
const float MIN_RANGE = 3.2;
...
void IADC_IRQHandler(void)
{
  ...
  // conversion value.
  singleResult = sample.data * 2.42 / 0xFFFF;

  // we have a sample so set PWM flag
  _sample_flag = 1;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // check flag
    if (_sample_flag) {_sample_flag = 0;}

    handle_pwm(singleResult);
